...
def write_test_file(test_cases: list[dict]) -> None:
    ...
    content = '''"""Auto-generated test suite for history readers.
...
'''
    ...
    for case in test_cases:
        if case["should_parse"]:
            content += f'    ({repr(case["input"])}, {repr(case["shell"])}, True, {repr(case["reason"])}),\n'
        else:
            content += f'    ({repr(case["input"])}, {repr(case["shell"])}, False, {repr(case["reason"])}),\n'
...
def test_reader_parse_line(raw_line, shell, should_parse, reason):
    ...
    if shell == "bash":
        ...
        if raw_line.startswith("#"):
            assert not raw_line[1:].strip().isdigit() or len(raw_line[1:].strip()) == 0
    elif shell == "zsh_extended":
        reader = ZshReader()
        if raw_line.startswith(": "):
            assert ";" in raw_line or not raw_line.rstrip().endswith("\\\\")
    elif shell == "fish":
        reader = FishReader()
...
def test_all_readers_skip_empty_lines():
    ...
    readers = [
        ...
        (ZshReader(), "zsh"),
        (FishReader(), "fish"),
        (PowerShellReader(), "powershell"),
    ]

    for reader, shell_name in readers:
        content = f"""cmd1
{chr(10)}cmd2"""
        entries = reader.parse(content)
        assert len(entries) == 2
